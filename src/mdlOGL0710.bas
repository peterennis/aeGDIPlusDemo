...
'- Does only work with Office 2007 and 2010! (see *Remark below)
'  Do not attempt to use in O2003 etc
...
'*Remark:
'We use here a special gdi+ version that comes with Office 2007/2010! (program files\common files\microsoft shared\office1x\ogl.dll)
...
Sub ShutDownGDIP()
    If lGDIP <> 0 Then
        ...
        If Not bSharedLoad Then FreeLibrary GetModuleHandle("ogl.dll")
    End If
End Sub
